...
int main(void) {
	...
	/* Captura de vídeo pela Webcam #0 */
	...
	while (key != 'q')
	{
		...
		vc_gray_to_binary(imageGray, imageBinary, 100); // Converter escala de cinzentos para binário
		vc_binary_open(imageBinary, imageOpen, 8, 8); // Aplicar uma abertura na imagem
		...
		if (blobs)
		{
			...
			// Inverter a imagem para poder fazer a bounding box à volta do sinal mostrado
			...
			if (auxBlobs)
			{
				...
				color = vc_bounding_box(imageOriginal, imageInverted, auxBlobs, auxNLabels, 200, 400, minB, maxB, minR, maxR, maxG_R, maxG_B); // widthMin, widthMax

				// Recontagem de labels para ignorar labels indesejados
				nlabels = vc_recount_labels(blobs, nlabels, 20, 300); // widthMin, widthMax
